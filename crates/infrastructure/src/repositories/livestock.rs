...
impl AnimalRepository for AnimalRepo {
    ...
    fn update(
        ...
        Box::pin(async move {
            ...
            if let Some(b) = dto.breed {
                update_doc.insert("breed", b);
            }
            if let Some(i) = dto.identifier {
                update_doc.insert("identifier", i);
            }
            if let Some(s) = dto.status {
                let s_val = mongodb::bson::to_bson(&s).unwrap_or(mongodb::bson::Bson::Null);
                update_doc.insert("status", s_val);
            }
            if let Some(sid) = dto.current_site_id {
                update_doc.insert("current_site_id", sid.to_string());
            }
            if let Some(gid) = dto.group_id {
                update_doc.insert("group_id", gid.to_string());
            }
    ...
    fn add_treatment(
        ...
        Box::pin(async move {
            let filter = doc! { "tenant_id": tid.to_string(), "id": id.to_string() };
            let t_doc =
                to_document(&treatment).map_err(|e| SharedError::Database(e.to_string()))?;
            let update = doc! {
