...
#[derive(Clone)]
pub struct HarvestSeasonRepo {
    base: MongoRepository<HarvestSeason>,
}
impl HarvestSeasonRepo {
    pub fn new(c: Collection<HarvestSeason>) -> Self {
        Self {
...
#[derive(Clone)]
pub struct HarvestLotRepo {
    base: MongoRepository<HarvestLot>,
}
impl HarvestLotRepo {
    pub fn new(c: Collection<HarvestLot>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
...
#[derive(Clone)]
pub struct HarvestDeliveryRepo {
    base: MongoRepository<HarvestDelivery>,
}
impl HarvestDeliveryRepo {
    pub fn new(c: Collection<HarvestDelivery>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
...
#[derive(Clone)]
pub struct ColdChainLogRepo {
    base: MongoRepository<ColdChainLog>,
}
impl ColdChainLogRepo {
    pub fn new(c: Collection<ColdChainLog>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
    pub fn find_all_for_lot(
        ...
        lot_id: Uuid,
        p: Pagination,
    ) -> Fut<PaginatedResponse<ColdChainLog>> {
        let c = self.base.collection.clone();
        Box::pin(async move {
            let filter = doc! { "tenant_id": tid.to_string(), "lot_id": lot_id.to_string() };
