...
#[derive(Clone)]
pub struct SiteRepo {
    base: MongoRepository<Site>,
}
impl SiteRepo {
    pub fn new(c: Collection<Site>) -> Self {
        Self {
    ...
    pub fn count(&self, tid: TenantId) -> Fut<u64> {
        ...
        Box::pin(async move {
            c.count_documents(doc! {"tenant_id":tid.to_string()})
                .await
    ...
    fn create(&self, tid: TenantId, dto: CreateSiteDto, by: Uuid) -> RepositoryFuture<Site> {
        ...
        Box::pin(async move {
            ...
            let s = Site {
                ...
                plots: dto.plots.unwrap_or_default(),
