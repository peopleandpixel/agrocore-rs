...
impl SiteRepository for SiteRepo {
    ...
    fn create(&self, tid: TenantId, dto: CreateSiteDto, by: Uuid) -> RepositoryFuture<Site> {
        ...
        Box::pin(async move {
            let now=Utc::now();
            let s=Site{id:Uuid::new_v4(),tenant_id:tid,business_id:None,label:dto.label,site_type:dto.site_type,crop_type:dto.crop_type,variety:dto.variety,area:dto.area,plots:dto.plots.unwrap_or_default(),row_config:dto.row_config,bbch_stage:dto.bbch_stage,planted_date:dto.planted_date,cleared_date:None,soil_type:dto.soil_type,slope:dto.slope,slope_facing:dto.slope_facing,altitude:dto.altitude,organic:dto.organic,organic_eligible:None,center:dto.center,sigpac_data:dto.sigpac_data,regepac_id:dto.regepac_id,boundary:dto.boundary,custom_fields:dto.custom_fields,note1:dto.note1,note2:dto.note2,is_active:true,is_temporary:false,created_at:now,updated_at:now,created_by:Some(by),updated_by:Some(by)};
