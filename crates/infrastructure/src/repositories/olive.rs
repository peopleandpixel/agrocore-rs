...
use agrocore_domain::entities::olive::{
    CreateOliveGroveDto, CreateOliveOilRecordDto, OliveGrove, OliveOilRecord,
};
...
#[derive(Clone)]
pub struct OliveGroveRepo {
    base: MongoRepository<OliveGrove>,
}
impl OliveGroveRepo {
    pub fn new(c: Collection<OliveGrove>) -> Self {
        Self {
...
#[derive(Clone)]
pub struct OliveOilRecordRepo {
    base: MongoRepository<OliveOilRecord>,
}
impl OliveOilRecordRepo {
    pub fn new(c: Collection<OliveOilRecord>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
    ...
    pub fn create(&self, tid: TenantId, dto: CreateOliveOilRecordDto) -> Fut<OliveOilRecord> {
        ...
        Box::pin(async move {
            let now = Utc::now();
            let rec = OliveOilRecord {
                id: Uuid::new_v4(),
                tenant_id: tid,
                grove_id: dto.grove_id,
                harvest_year: dto.harvest_year,
                oil_grade: dto.oil_grade,
                acidity_pct: dto.acidity_pct,
                peroxide_value: dto.peroxide_value,
                sensory_score: dto.sensory_score,
                liters_produced: dto.liters_produced,
                mill_name: dto.mill_name,
