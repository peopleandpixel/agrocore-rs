...
use agrocore_domain::entities::compliance::{
    AuditLog, ComplianceChecklist, CreateComplianceChecklistDto, CreateFertilizerRecordDto,
    FertilizerRecord,
};
use agrocore_domain::entities::plant_protection::{
    ApplicatorLicense, CreatePlantProtectionDto, PlantProtectionRecord,
};
...
pub struct AuditLogRepo {
    base: MongoRepository<AuditLog>,
}
impl AuditLogRepo {
    pub fn new(c: Collection<AuditLog>) -> Self {
        Self {
...
#[derive(Clone)]
pub struct ComplianceChecklistRepo {
    base: MongoRepository<ComplianceChecklist>,
}
impl ComplianceChecklistRepo {
    pub fn new(c: Collection<ComplianceChecklist>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
    ...
    ) -> Fut<ComplianceChecklist> {
        let c = self.base.collection.clone();
        Box::pin(async move {
            let now = Utc::now();
            let cl = ComplianceChecklist {
                id: Uuid::new_v4(),
                tenant_id: tid,
                site_id: dto.site_id,
                checklist_type: dto.checklist_type,
                status: agrocore_domain::entities::compliance::ComplianceStatus::Pending,
                items: Vec::new(),
...
#[derive(Clone)]
pub struct FertilizerRecordRepo {
    base: MongoRepository<FertilizerRecord>,
}
impl FertilizerRecordRepo {
    pub fn new(c: Collection<FertilizerRecord>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
...
#[derive(Clone)]
pub struct PlantProtectionRecordRepo {
    base: MongoRepository<PlantProtectionRecord>,
}
impl PlantProtectionRecordRepo {
    pub fn new(c: Collection<PlantProtectionRecord>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
    ...
    ) -> Fut<PlantProtectionRecord> {
        let c = self.base.collection.clone();
        Box::pin(async move {
            let now = Utc::now();
            let rec = PlantProtectionRecord {
                id: Uuid::new_v4(),
                tenant_id: tid,
                site_id: dto.site_id,
                order_id: dto.order_id,
...
#[derive(Clone)]
pub struct ApplicatorLicenseRepo {
    base: MongoRepository<ApplicatorLicense>,
}
impl ApplicatorLicenseRepo {
    pub fn new(c: Collection<ApplicatorLicense>) -> Self {
        Self {
            base: MongoRepository::new(c),
        }
    }
    ...
    pub fn find_all(
        ...
        tid: TenantId,
        p: Pagination,
    ) -> RepositoryFuture<PaginatedResponse<ApplicatorLicense>> {
        let c = self.base.collection.clone();
        Box::pin(async move {
            let f = doc! {"tenant_id":tid.to_string()};
