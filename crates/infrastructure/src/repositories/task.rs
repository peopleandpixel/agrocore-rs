...
#[derive(Clone)]
pub struct TaskDataRepo {
    base: MongoRepository<TaskData>,
}
impl TaskDataRepo {
    pub fn new(c: Collection<TaskData>) -> Self {
        Self {
    ...
    pub fn find_by_order(&self, tid: TenantId, oid: Uuid) -> Fut<Vec<TaskData>> {
        ...
        Box::pin(async move {
            ...
            while let Some(r) = cur.next().await {
                d.push(r.map_err(|e| SharedError::Database(e.to_string()))?);
            }
            Ok(d)
        })
    }
    pub fn find_by_worker(
        &self,
        tid: TenantId,
        wid: Uuid,
        p: Pagination,
    ...
    pub fn create(&self, tid: TenantId, wid: Uuid, dto: CreateTaskDataDto) -> Fut<TaskData> {
        ...
        Box::pin(async move {
            ...
            let td = TaskData {
                ...
                materials_used: dto.materials_used,
                observations: dto.observations,
                gps_track: dto.gps_track,
    ...
    pub fn delete(&self, tid: TenantId, id: Uuid) -> Fut<bool> {
        ...
        Box::pin(async move {
            let r = c
                .delete_one(doc! {"tenant_id":tid.to_string(),"id":id.to_string()})
                .await
                .map_err(|e| SharedError::Database(e.to_string()))?;
