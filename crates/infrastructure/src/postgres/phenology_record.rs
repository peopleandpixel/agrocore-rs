use agrocore_domain::entities::weather::PhenologyRecord;
use agrocore_domain::entities::tenant::TenantId;
use agrocore_domain::repositories::{PaginatedResponse, Pagination, RepositoryFuture, PhenologyRecordRepo};
...
impl PhenologyRecordRepo for PgPhenologyRecordRepo {
    ...
    fn find_all(&self, tid: TenantId, p: Pagination) -> RepositoryFuture<PaginatedResponse<PhenologyRecord>> {
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM phenology_records WHERE tenant_id = $1::uuid")
                .bind(tid.to_string()).fetch_one(&pool).await
                .map_err(|e| SharedError::Database(e.to_string()))?;
            let data: Vec<PhenologyRecord> = sqlx::query_as("SELECT * FROM phenology_records WHERE tenant_id = $1::uuid LIMIT $2 OFFSET $3")
                .bind(tid.to_string()).bind(per_page as i32).bind(offset as i32)
    ...
    fn create(&self, _tid: TenantId, _dto: agrocore_domain::entities::weather::CreatePhenologyRecordDto) -> RepositoryFuture<PhenologyRecord> {
