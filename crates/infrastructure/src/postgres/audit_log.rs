...
impl AuditLogRepo for PgAuditLogRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<AuditLog>> {
        ...
        Box::pin(async move {
            sqlx::query_as::<_, AuditLog>("SELECT * FROM audit_logs WHERE tenant_id = $1::uuid AND id = $2")
                .bind(tid.to_string())
                .bind(id)
                .fetch_optional(&pool)
                .await
    ...
    fn find_all(&self, tid: TenantId, p: Pagination) -> RepositoryFuture<PaginatedResponse<AuditLog>> {
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM audit_logs WHERE tenant_id = $1::uuid")
                .bind(tid.to_string())
                .fetch_one(&pool)
                .await
                .map_err(|e| SharedError::Database(e.to_string()))?;
