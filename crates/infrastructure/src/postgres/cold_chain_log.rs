...
impl ColdChainLogRepo for PgColdChainLogRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<ColdChainLog>> {
        ...
        Box::pin(async move {
            sqlx::query_as::<_, ColdChainLog>("SELECT * FROM cold_chain_logs WHERE id = $1 AND tenant_id = $2")
                .bind(id)
                .bind(tid.to_string())
                .fetch_optional(&pool)
                .await
    ...
    fn find_all(&self, tid: TenantId, p: Pagination) -> RepositoryFuture<PaginatedResponse<ColdChainLog>> {
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM cold_chain_logs WHERE tenant_id = $1::uuid")
                .bind(tid.to_string())
                .fetch_one(&pool)
                .await
                .map_err(|e| SharedError::Database(e.to_string()))?;
    ...
    fn find_by_lot(&self, tid: TenantId, lot_id: Uuid, p: Pagination) -> RepositoryFuture<PaginatedResponse<ColdChainLog>> {
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM cold_chain_logs WHERE tenant_id = $1::uuid AND lot_id = $2")
                .bind(tid.to_string())
                .bind(lot_id)
                .fetch_one(&pool)
                .await
    ...
    fn update(&self, tid: TenantId, id: Uuid, dto: UpdateColdChainLogDto) -> RepositoryFuture<Option<ColdChainLog>> {
        let pool = self.pool.clone();
        Box::pin(async move {
            sqlx::query_as::<_, ColdChainLog>(
