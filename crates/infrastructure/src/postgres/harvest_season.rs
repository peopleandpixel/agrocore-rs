...
impl HarvestSeasonRepo for PgHarvestSeasonRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<HarvestSeason>> {
        ...
        Box::pin(async move {
            sqlx::query_as::<_, HarvestSeason>("SELECT * FROM harvest_seasons WHERE id = $1 AND tenant_id = $2")
                .bind(id)
                .bind(tid.to_string())
                .fetch_optional(&pool)
                .await
    ...
    fn create(&self, tid: TenantId, dto: CreateHarvestSeasonDto, _by: Uuid) -> RepositoryFuture<HarvestSeason> {
        let pool = self.pool.clone();
        Box::pin(async move {
            let id = Uuid::new_v4();
    ...
    fn update(&self, tid: TenantId, id: Uuid, dto: UpdateHarvestSeasonDto, _by: Uuid) -> RepositoryFuture<Option<HarvestSeason>> {
        let pool = self.pool.clone();
        Box::pin(async move {
            sqlx::query_as::<_, HarvestSeason>(
