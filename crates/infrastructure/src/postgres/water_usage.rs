...
impl WaterUsageRepo for PgWaterUsageRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<WaterUsage>> {
        ...
        Box::pin(async move {
            sqlx::query_as::<_, WaterUsage>("SELECT * FROM water_usages WHERE tenant_id = $1::uuid AND id = $2")
                .bind(tid.to_string())
                .bind(id)
                .fetch_optional(&pool)
                .await
    ...
    fn find_all(&self, tid: TenantId, p: Pagination) -> RepositoryFuture<PaginatedResponse<WaterUsage>> {
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM water_usages WHERE tenant_id = $1::uuid")
                .bind(tid.to_string())
                .fetch_one(&pool)
                .await
                .map_err(|e| SharedError::Database(e.to_string()))?;
    ...
    fn create(&self, tid: TenantId, dto: agrocore_domain::entities::water::CreateWaterUsageDto, _by: Uuid) -> RepositoryFuture<WaterUsage> {
        let pool = self.pool.clone();
        Box::pin(async move {
    ...
    fn update(&self, tid: TenantId, id: Uuid, dto: agrocore_domain::entities::water::UpdateWaterUsageDto, _by: Uuid) -> RepositoryFuture<Option<WaterUsage>> {
        let pool = self.pool.clone();
        ...
        Box::pin(async move {
            ...
            if dto.source_id.is_some() { parts.push(format!("source_id = ${}", idx + 2)); idx += 1; }
            if dto.site_id.is_some() { parts.push(format!("site_id = ${}", idx + 2)); idx += 1; }
            if dto.usage_date.is_some() { parts.push(format!("usage_date = ${}", idx + 2)); idx += 1; }
            if dto.volume_m3.is_some() { parts.push(format!("volume_m3 = ${}", idx + 2)); idx += 1; }
            if dto.irrigation_method.is_some() { parts.push(format!("irrigation_method = ${}", idx + 2)); idx += 1; }
            if dto.efficiency_pct.is_some() { parts.push(format!("efficiency_pct = ${}", idx + 2)); }
            
            if parts.is_empty() { return self_clone.find_by_id(tid, id).await; }
            ...
            if let Some(v) = dto.source_id { q = q.bind(v); }
            if let Some(v) = dto.site_id { q = q.bind(v); }
            if let Some(v) = dto.usage_date { q = q.bind(v); }
            if let Some(v) = dto.volume_m3 { q = q.bind(v); }
            if let Some(v) = dto.irrigation_method { q = q.bind(serde_json::to_value(v).unwrap()); }
            if let Some(v) = dto.efficiency_pct { q = q.bind(v); }
    ...
    fn delete(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<bool> {
        let pool = self.pool.clone();
        Box::pin(async move {
            let res = sqlx::query("DELETE FROM water_usages WHERE tenant_id = $1::uuid AND id = $2")
                .bind(tid.to_string())
                .bind(id)
                .execute(&pool)
                .await
    ...
    fn find_by_source(&self, tid: TenantId, source_id: Uuid, p: Pagination) -> RepositoryFuture<PaginatedResponse<WaterUsage>> {
        ...
        let offset = page * per_page;
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM water_usages WHERE tenant_id = $1::uuid AND source_id = $2")
                .bind(tid.to_string())
                .bind(source_id)
                .fetch_one(&pool)
                .await
    ...
    fn find_by_site(&self, tid: TenantId, site_id: Uuid, p: Pagination) -> RepositoryFuture<PaginatedResponse<WaterUsage>> {
        ...
        let offset = page * per_page;
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM water_usages WHERE tenant_id = $1::uuid AND site_id = $2")
                .bind(tid.to_string())
                .bind(site_id)
                .fetch_one(&pool)
                .await
