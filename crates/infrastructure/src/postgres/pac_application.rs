...
use agrocore_domain::repositories::{PACApplicationRepo, PaginatedResponse, Pagination, RepositoryFuture, AuditLogRepo};
...
impl PACApplicationRepo for PgPACApplicationRepo {
    ...
    fn update(&self, tid: TenantId, id: Uuid, dto: UpdatePACApplicationDto, by: Uuid) -> RepositoryFuture<Option<PACApplication>> {
        let pool = self.pool.clone();
        let audit_repo = PgAuditLogRepo::new(pool.clone());
        Box::pin(async move {
            let old_val = sqlx::query_as::<_, PACApplication>("SELECT * FROM pac_applications WHERE tenant_id = $1::uuid AND id = $2")
                .bind(tid.to_string())
                .bind(id)
                .fetch_optional(&pool)
                .await
            ...
            let new_val = q.fetch_optional(&pool).await.map_err(|e| SharedError::Database(e.to_string()))?;
