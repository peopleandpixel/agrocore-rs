...
impl TenantRepository for PgTenantRepo {
    fn find_by_id(&self, id: TenantId) -> RepositoryFuture<Option<Tenant>> {
        ...
        Box::pin(async move {
            let row = sqlx::query_as::<_, (Tenant,)>("SELECT row_to_json(tenants) FROM tenants WHERE id = $1")
                .bind(id.to_string())
                .fetch_optional(&pool)
                .await
    ...
    fn find_all(&self, p: Pagination) -> RepositoryFuture<PaginatedResponse<Tenant>> {
        ...
        Box::pin(async move {
            ...
            let items: Vec<Tenant> = sqlx::query_as("SELECT * FROM tenants ORDER BY name LIMIT $1 OFFSET $2")
                .bind(p.limit as i32)
