...
use agrocore_domain::repositories::{WaterSourceRepo, RepositoryFuture, PaginatedResponse, Pagination};
...
impl WaterSourceRepo for PgWaterSourceRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<WaterSource>> {
        ...
        Box::pin(async move {
            let row = sqlx::query_as("SELECT * FROM water_sources WHERE id = $1::uuid AND tenant_id = $2::uuid")
                .bind(id)
                .bind(tid.to_string())
                .fetch_optional(&pool)
                .await
    ...
    fn find_all(&self, tid: TenantId, p: Pagination) -> RepositoryFuture<PaginatedResponse<WaterSource>> {
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM water_sources WHERE tenant_id = $1::uuid")
                .bind(tid.to_string())
                .fetch_one(&pool)
                .await
                .map_err(|e| SharedError::Database(e.to_string()))?;
            let data: Vec<WaterSource> = sqlx::query_as("SELECT * FROM water_sources WHERE tenant_id = $1::uuid LIMIT $2 OFFSET $3")
                .bind(tid.to_string())
    ...
    fn create(&self, tid: TenantId, dto: agrocore_domain::entities::water::CreateWaterSourceDto, _by: Uuid) -> RepositoryFuture<WaterSource> {
        let pool = self.pool.clone();
        let id = Uuid::new_v4();
        Box::pin(async move {
    ...
    fn find_by_site(&self, _tid: TenantId, _site_id: Uuid, _p: Pagination) -> RepositoryFuture<PaginatedResponse<agrocore_domain::entities::water::WaterSource>> {
        Box::pin(async move { Err(SharedError::Internal("Not implemented".into())) })
    }
    fn update(&self, _tid: TenantId, _id: Uuid, _dto: agrocore_domain::entities::water::UpdateWaterSourceDto, _by: Uuid) -> RepositoryFuture<Option<agrocore_domain::entities::water::WaterSource>> {
        Box::pin(async move { Err(SharedError::Internal("Not implemented".into())) })
    }
