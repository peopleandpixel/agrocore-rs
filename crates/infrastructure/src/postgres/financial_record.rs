...
impl FinancialRecordRepo for PgFinancialRecordRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<FinancialRecord>> {
        ...
        Box::pin(async move {
            sqlx::query_as::<_, FinancialRecord>("SELECT * FROM financial_records WHERE id = $1 AND tenant_id = $2")
                .bind(id)
                .bind(tid.to_string())
                .fetch_optional(&pool)
                .await
    ...
    fn find_by_cost_center(&self, _tid: TenantId, _cost_center_id: Uuid, _p: Pagination) -> RepositoryFuture<PaginatedResponse<FinancialRecord>> {
        Box::pin(async move { Err(SharedError::Internal("Not implemented".to_string())) })
    }
    fn create(&self, tid: TenantId, dto: CreateFinancialRecordDto, _by: Uuid) -> RepositoryFuture<FinancialRecord> {
        let pool = self.pool.clone();
        Box::pin(async move {
    ...
    fn update(&self, _tid: TenantId, _id: Uuid, _dto: agrocore_domain::entities::finance::UpdateFinancialRecordDto, _by: Uuid) -> RepositoryFuture<Option<FinancialRecord>> {
        Box::pin(async move { Err(SharedError::Internal("Not implemented".to_string())) })
    }
