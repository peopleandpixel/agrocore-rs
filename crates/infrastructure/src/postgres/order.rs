...
use agrocore_domain::repositories::{PaginatedResponse, Pagination, RepositoryFuture, OrderRepository, AuditLogRepo};
...
impl OrderRepository for PgOrderRepo {
    ...
    fn find_by_id_visible(&self, tid: TenantId, id: Uuid, user_id: Uuid, roles: &[UserRole]) -> RepositoryFuture<Option<Order>> {
        let pool = self.pool.clone();
        ...
        Box::pin(async move {
            ...
            if can_see_all {
                ...
                sqlx::query_as::<_, Order>("SELECT * FROM orders WHERE id = $1 AND tenant_id = $2 AND assigned_to = $3")
                    .bind(id)
                    .bind(tid.to_string())
                    .bind(user_id)
                    .fetch_optional(&pool)
    ...
    fn find_all_visible(&self, tid: TenantId, p: Pagination, _user_id: Uuid, _roles: &[UserRole]) -> RepositoryFuture<PaginatedResponse<Order>> {
        self.find_all(tid, p)
    }
    ...
    fn update(&self, tid: TenantId, id: Uuid, dto: UpdateOrderDto, by: Uuid) -> RepositoryFuture<Option<Order>> {
        ...
        Box::pin(async move {
            ...
            let old_order = sqlx::query_as::<_, Order>("SELECT * FROM orders WHERE id = $1 AND tenant_id = $2")
                .bind(id)
                .bind(tid.to_string())
                .fetch_optional(&pool)
                .await
    ...
    fn find_assigned_to_worker(&self, tid: Uuid, worker_id: Uuid) -> RepositoryFuture<Vec<Order>> {
        let pool = self.pool.clone();
        Box::pin(async move {
            sqlx::query_as::<_, Order>("SELECT * FROM orders WHERE tenant_id = $1::uuid AND assigned_to = $2::uuid")
                .bind(tid.to_string())
