...
impl WorkLogRepo for PgWorkLogRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<WorkLog>> {
        ...
        Box::pin(async move {
            sqlx::query_as::<_, WorkLog>("SELECT * FROM work_logs WHERE tenant_id = $1::uuid AND id = $2")
                .bind(tid.to_string())
                .bind(id)
                .fetch_optional(&pool)
                .await
    ...
    fn find_all(&self, tid: TenantId, p: Pagination) -> RepositoryFuture<PaginatedResponse<WorkLog>> {
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM work_logs WHERE tenant_id = $1::uuid")
                .bind(tid.to_string())
                .fetch_one(&pool)
                .await
                .map_err(|e| SharedError::Database(e.to_string()))?;
    ...
    fn update(&self, tid: TenantId, id: Uuid, dto: UpdateWorkLogDto, _by: Uuid) -> RepositoryFuture<Option<WorkLog>> {
        let pool = self.pool.clone();
        ...
        Box::pin(async move {
            ...
            if dto.date.is_some() { parts.push(format!("date = ${}", idx + 2)); idx += 1; }
            if dto.hours_worked.is_some() { parts.push(format!("hours_worked = ${}", idx + 2)); idx += 1; }
            if dto.overtime_hours.is_some() { parts.push(format!("overtime_hours = ${}", idx + 2)); idx += 1; }
            if dto.rest_period_hours.is_some() { parts.push(format!("rest_period_hours = ${}", idx + 2)); idx += 1; }
            if dto.task_description.is_some() { parts.push(format!("task_description = ${}", idx + 2)); idx += 1; }
            if dto.site_id.is_some() { parts.push(format!("site_id = ${}", idx + 2)); idx += 1; }
            if dto.is_night_shift.is_some() { parts.push(format!("is_night_shift = ${}", idx + 2)); idx += 1; }
            if dto.breaks_taken.is_some() { parts.push(format!("breaks_taken = ${}", idx + 2)); }
            ...
            if let Some(v) = dto.date { q = q.bind(v); }
            if let Some(v) = dto.hours_worked { q = q.bind(v); }
            if let Some(v) = dto.overtime_hours { q = q.bind(v); }
            if let Some(v) = dto.rest_period_hours { q = q.bind(v); }
            if let Some(v) = dto.task_description { q = q.bind(v); }
            if let Some(v) = dto.site_id { q = q.bind(v); }
            if let Some(v) = dto.is_night_shift { q = q.bind(v); }
            if let Some(v) = dto.breaks_taken { q = q.bind(v as i32); }
    ...
    fn find_by_worker(&self, tid: TenantId, worker_id: Uuid, p: Pagination) -> RepositoryFuture<PaginatedResponse<WorkLog>> {
        ...
        let offset = page * per_page;
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM work_logs WHERE tenant_id = $1::uuid AND worker_id = $2")
                .bind(tid.to_string())
                .bind(worker_id)
                .fetch_one(&pool)
                .await
