use agrocore_domain::entities::plant_protection::{CreatePlantProtectionDto, PlantProtectionRecord, UpdatePlantProtectionDto, ApplicatorLicense, CreateApplicatorLicenseDto, UpdateApplicatorLicenseDto};
...
impl PlantProtectionRecordRepo for PgPlantProtectionRecordRepo {
    fn find_by_id(&self, tid: TenantId, id: Uuid) -> RepositoryFuture<Option<PlantProtectionRecord>> {
        ...
        Box::pin(async move {
            sqlx::query_as::<_, PlantProtectionRecord>("SELECT * FROM plant_protection_records WHERE id = $1 AND tenant_id = $2")
                .bind(id)
                .bind(tid.to_string())
                .fetch_optional(&pool)
                .await
    ...
    fn create(&self, tid: TenantId, dto: CreatePlantProtectionDto, _by: Uuid) -> RepositoryFuture<PlantProtectionRecord> {
        let pool = self.pool.clone();
        Box::pin(async move {
            let id = Uuid::new_v4();
    ...
    fn update(&self, tid: TenantId, id: Uuid, dto: UpdatePlantProtectionDto, _by: Uuid) -> RepositoryFuture<Option<PlantProtectionRecord>> {
        let pool = self.pool.clone();
        Box::pin(async move {
            sqlx::query_as::<_, PlantProtectionRecord>(
    ...
    fn find_applicator_license_by_user(&self, _tid: TenantId, _user_id: Uuid) -> RepositoryFuture<Option<ApplicatorLicense>> {
        Box::pin(async move { Err(SharedError::Internal("Not implemented".into())) })
    }

    fn create_applicator_license(&self, _tid: TenantId, _dto: CreateApplicatorLicenseDto) -> RepositoryFuture<ApplicatorLicense> {
        Box::pin(async move { Err(SharedError::Internal("Not implemented".into())) })
    }

    fn update_applicator_license(&self, _tid: TenantId, _id: Uuid, _dto: UpdateApplicatorLicenseDto) -> RepositoryFuture<Option<ApplicatorLicense>> {
        Box::pin(async move { Err(SharedError::Internal("Not implemented".into())) })
    }
}
