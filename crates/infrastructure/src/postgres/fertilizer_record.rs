use agrocore_domain::entities::fertilizer::{
    CreateFertilizerRecordDto, FertilizerRecord, UpdateFertilizerRecordDto,
};
use agrocore_domain::entities::tenant::TenantId;
use agrocore_domain::repositories::{
    FertilizerRecordRepo, PaginatedResponse, Pagination, RepositoryFuture,
};
...
impl FertilizerRecordRepo for PgFertilizerRecordRepo {
    ...
    fn find_all(
        ...
        Box::pin(async move {
            let total: i64 =
                sqlx::query_scalar("SELECT COUNT(*) FROM fertilizer_records WHERE tenant_id = $1")
                    .bind(tid)
                    .fetch_one(&pool)
                    .await
                    .map_err(|e| SharedError::Database(e.to_string()))?;
    ...
    fn find_by_site(
        ...
        Box::pin(async move {
            let total: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM fertilizer_records WHERE tenant_id = $1 AND site_id = $2",
            )
            .bind(tid)
            .bind(site_id)
            .fetch_one(&pool)
