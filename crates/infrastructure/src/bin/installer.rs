...
fn main() -> io::Result<()> {
    ...
    let services = vec![
        (
            "agrocore-api",
            "AgroCore REST API & Admin UI Service",
            "/usr/local/bin/agrocore-api",
        ),
        (
            "agrocore-weather",
            "AgroCore Weather Service",
            "/usr/local/bin/agrocore-weather",
        ),
        (
            "agrocore-reporting",
            "AgroCore Reporting Service",
            "/usr/local/bin/agrocore-reporting",
        ),
    ];
...
"#,
    ...
    println!("\nNächste Schritte:");
    println!(
        "1. Benutzer anlegen: sudo useradd -r -m -d {} -s /bin/false {}",
        data_dir, user
    );
