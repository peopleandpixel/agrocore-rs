...
pub fn TaskDetailPage() -> impl IntoView {
    ...
    let task_id_str = path.rsplit('/').next().unwrap_or("");
    let tid = match uuid::Uuid::parse_str(task_id_str) {
        Ok(id) => id,
        Err(_) => {
            let msg = crate::t!(t, "invalid_task_id").to_string();
            return view! {
                <div class="p-4"><div class="alert alert-error"><span>{msg}</span></div></div>
            }.into_view();
        }
    };
    ...
    let on_back = move |_: ev::ClickEvent| { let _ = navigate("/tasks".to_string()); };

    view! {
        <div class="flex flex-col gap-6">
            <div class="flex items-center gap-4">
                <button class="btn btn-ghost btn-sm" on:click=on_back>
            ...
            {move || {
                match (loading.get(), error.get().as_ref(), task.get()) {
                            ...
                            <div class="card bg-base-100 shadow">
                                <div class="card-body">
                                    ...
                                    <div class="card-actions justify-start mt-6">
                                        <button class="btn btn-outline btn-primary" on:click=move |_: ev::ClickEvent| {
                                            set_error.set(None);
                                        ...
                                        <button class="btn btn-outline btn-warning" on:click=move |_: ev::ClickEvent| {
                                            spawn_local(async move {
                                        ...
                                        <button class="btn btn-outline btn-accent" on:click=move |_: ev::ClickEvent| {
                                            spawn_local(async move {
                                        ...
                                        <button class="btn btn-outline btn-success" on:click=move |_: ev::ClickEvent| {
                                            spawn_local(async move {
                    ...
                    (false, Some(_), None) => view! {
                        <div class="alert alert-info"><span>{crate::t!(t, "no_task_found")}</span></div>
                    }.into_any(),
                }
            }}
        </div>
