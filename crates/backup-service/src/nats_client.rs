...
impl NatsClient {
    ...
    pub async fn publish_backup_failed(&self, backup_type: &str, error: &str) {
        ...
        self.publish_event("backup.failed".to_string(), &event)
            .await;
    }

    async fn publish_event(&self, subject: String, event: &BackupEvent) {
