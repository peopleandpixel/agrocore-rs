//! Poland LPIS Provider
//!
//! Poland's LPIS system managed by ARiMR (Agencja Restrukturyzacji i Modernizacji Rolnictwa).
//! Data available via ARiMR portal and INSPIRE geoportal.
...
const LPIS_PL_WFS_URL: &str = "https://geoportal.arrim.gov.pl/wfs";

#[derive(Debug, Error)]
pub enum LpisPlError {
...
struct LpisPlParcel {
    ...
    #[serde(rename = "obreb")]
    obreb: Option<String>,
    #[serde(rename = "dzialka")]
    dzialka: Option<String>,
    #[serde(rename = "uprawa")]
    uprawa: Option<String>,
    #[serde(rename = "powierzchnia")]
    powierzchnia: Option<f64>,
...
impl PolishLpisProvider {
    pub fn new() -> Self {
        ...
        Self {
            client,
            base_url: LPIS_PL_WFS_URL.to_string(),
        }
    }

    fn build_query_url(&self, query: &agrocore_shared::lpis::LpisQuery) -> String {
        let mut url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=LPIS&outputFormat=application/json",
            self.base_url
        );

        let mut filters = Vec::new();

        if let Some(woj) = &query.province {
            filters.push(format!("wojewodztwo='{}'", woj));
        }

        if let Some(powiat) = &query.municipality {
            filters.push(format!("powiat='{}'", powiat));
        }

        if let Some(gmina) = &query.aggregate {
            filters.push(format!("gmina='{}'", gmina));
        }

        if let Some(uprawa) = &query.reference {
            filters.push(format!("uprawa='{}'", uprawa));
        }
    ...
    fn gml_to_polygon(&self, gml: &GmlGeometry) -> Result<Polygon<f64>, LpisPlError> {
        ...
        if !coords.len().is_multiple_of(2) {
            return Err(LpisPlError::Geometry(
    ...
    fn parcel_to_lpis(
        ...
        parcel: &LpisPlParcel,
        tenant_id: Uuid,
    ) -> Result<agrocore_shared::lpis::LpisParcel, LpisPlError> {
        let polygon = self.gml_to_polygon(&parcel.geometria)?;
        let geometry = Geometry::Polygon(polygon);

        let geo_json: GeoJson = GeoJsonGeometry::from(&geometry).into();
        let geometry_value =
            serde_json::to_value(geo_json).map_err(|e| LpisPlError::Geometry(e.to_string()))?;

        let area_ha = parcel.powierzchnia.map(|a| a / 10000.0);
        ...
        Ok(agrocore_shared::lpis::LpisParcel {
            ...
            province: parcel.wojewodztwo.clone(),
            municipality: parcel.gmina.clone(),
            aggregate: parcel.powiat.clone(),
            zone: parcel.obreb.clone(),
            polygon: parcel.dzialka.clone(),
            parcel: parcel.dzialka.clone(),
            enclosure: None,
            usage_code: parcel.uprawa.clone(),
            usage_description: None,
            geometry: geometry_value,
            area_hectares: area_ha,
            official_area_ha: area_ha,
            source_dataset: Some("LPIS-PL".to_string()),
...
#[async_trait]
impl LpisProvider for PolishLpisProvider {
    ...
    async fn search_near_point(
        ...
        let url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=LPIS&outputFormat=application/json&CQL_FILTER={}&count=100",
            self.base_url,
    ...
    async fn validate_geometry(
        ...
        Ok(agrocore_shared::lpis::LpisValidationResult {
            ...
            warnings: if is_empty {
                vec!["No matching LPIS-PL parcels found".to_string()]
            } else {
                vec![]
            },
        })
...
impl Default for PolishLpisProvider {
    fn default() -> Self {
        Self::new()
    }
}
