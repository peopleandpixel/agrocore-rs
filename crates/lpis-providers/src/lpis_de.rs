...
struct LpisDeParcel {
    ...
    #[serde(rename = "flurstueck")]
    flurstueck: Option<String>,
...
impl GermanLpisProvider {
    ...
    fn build_query_url(&self, query: &agrocore_shared::lpis::LpisQuery) -> String {
        ...
        if let Some(kreis) = &query.municipality {
            filters.push(format!("kreis='{}'", kreis));
        }

        if let Some(flurstueck) = &query.reference {
            filters.push(format!("flurstueck='{}'", flurstueck));
        }
    ...
    fn gml_to_polygon(&self, gml: &GmlGeometry) -> Result<Polygon<f64>, LpisDeError> {
        ...
        if !coords.len().is_multiple_of(2) {
            return Err(LpisDeError::Geometry(
    ...
    fn parcel_to_lpis(
        ...
        parcel: &LpisDeParcel,
        tenant_id: Uuid,
    ) -> Result<agrocore_shared::lpis::LpisParcel, LpisDeError> {
        let polygon = self.gml_to_polygon(&parcel.geometrie)?;
        let geometry = Geometry::Polygon(polygon);

        let geo_json: GeoJson = GeoJsonGeometry::from(&geometry).into();
        let geometry_value =
            serde_json::to_value(geo_json).map_err(|e| LpisDeError::Geometry(e.to_string()))?;
        ...
        Ok(agrocore_shared::lpis::LpisParcel {
            id,
            tenant_id,
            country: LpisCountry::Other, // Germany uses Other as no specific enum
            reference: parcel.id.clone(),
            province: parcel.bundesland.clone(),
            municipality: parcel.gemeinde.clone(),
            aggregate: parcel.kreis.clone(),
            zone: None,
            polygon: parcel.flurstueck.clone(),
            parcel: parcel.flurstueck.clone(),
            enclosure: None,
            ...
            source_dataset: Some("LPIS-DE".to_string()),
...
#[async_trait]
impl LpisProvider for GermanLpisProvider {
    fn country(&self) -> LpisCountry {
        LpisCountry::Other // Germany uses Other since not in enum
    }
    ...
    async fn list_parcels(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| format!("LPIS-DE request failed: {}", e))?;

        if !response.status().is_success() {
            return Err(format!("LPIS-DE API error: {}", response.status()));
        }
    ...
    async fn search_near_point(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| format!("LPIS-DE spatial search failed: {}", e))?;
    ...
    async fn validate_geometry(
        ...
        Ok(agrocore_shared::lpis::LpisValidationResult {
            ...
            warnings: if is_empty {
                vec!["No matching LPIS-DE parcels found".to_string()]
            } else {
                vec![]
            },
        })
