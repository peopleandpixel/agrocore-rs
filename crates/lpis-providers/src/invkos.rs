//! Austria INVEKOS (Integriertes Verwaltungs- und Kontrollsystem) Provider
//!
//! Austria's LPIS system managed by AMA (Agrarmarkt Austria) and federal states.
//! Data available via data.gv.at and state geoportals.
...
const INVEKOS_WFS_URL: &str = "https://data.gv.at/wfs";

#[derive(Debug, Error)]
pub enum InvekosError {
...
#[derive(Debug, Deserialize, Serialize)]
struct WfsFeature {
    #[serde(rename = "INVEKOS")]
    parcels: Vec<InvekosParcel>,
}

#[derive(Debug, Deserialize, Serialize)]
struct InvekosParcel {
    #[serde(rename = "id")]
    id: String,
    #[serde(rename = "bundesland")]
    bundesland: Option<String>,
    #[serde(rename = "bezirk")]
    bezirk: Option<String>,
    #[serde(rename = "gemeinde")]
    gemeinde: Option<String>,
    #[serde(rename = "katastralgemeinde")]
    katastralgemeinde: Option<String>,
    #[serde(rename = "grundstueck")]
    grundstueck: Option<String>,
    #[serde(rename = "nutzung")]
    nutzung: Option<String>,
...
#[derive(Clone)]
pub struct InvekosProvider {
    client: Client,
    base_url: String,
}

impl InvekosProvider {
    pub fn new() -> Self {
        ...
        Self {
            client,
            base_url: INVEKOS_WFS_URL.to_string(),
        }
    }

    fn build_query_url(&self, query: &agrocore_shared::lpis::LpisQuery) -> String {
        let mut url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=INVEKOS&outputFormat=application/json",
            self.base_url
        );

        let mut filters = Vec::new();

        if let Some(bundesland) = &query.province {
            filters.push(format!("bundesland='{}'", bundesland));
        }

        if let Some(bezirk) = &query.municipality {
            filters.push(format!("bezirk='{}'", bezirk));
        }

        if let Some(gemeinde) = &query.aggregate {
            filters.push(format!("gemeinde='{}'", gemeinde));
        }

        if let Some(nutzung) = &query.reference {
            filters.push(format!("nutzung='{}'", nutzung));
        }
    ...
    fn gml_to_polygon(&self, gml: &GmlGeometry) -> Result<Polygon<f64>, InvekosError> {
        ...
        if !coords.len().is_multiple_of(2) {
            return Err(InvekosError::Geometry(
    ...
    fn parcel_to_lpis(
        &self,
        parcel: &InvekosParcel,
        tenant_id: Uuid,
    ) -> Result<agrocore_shared::lpis::LpisParcel, InvekosError> {
        let polygon = self.gml_to_polygon(&parcel.geometrie)?;
        let geometry = Geometry::Polygon(polygon);

        let geo_json: GeoJson = GeoJsonGeometry::from(&geometry).into();
        let geometry_value =
            serde_json::to_value(geo_json).map_err(|e| InvekosError::Geometry(e.to_string()))?;

        let area_ha = parcel.flaeche.map(|a| a / 10000.0);
        ...
        Ok(agrocore_shared::lpis::LpisParcel {
            ...
            province: parcel.bundesland.clone(),
            municipality: parcel.gemeinde.clone(),
            aggregate: parcel.bezirk.clone(),
            zone: parcel.katastralgemeinde.clone(),
            polygon: parcel.grundstueck.clone(),
            parcel: parcel.grundstueck.clone(),
            enclosure: None,
            usage_code: parcel.nutzung.clone(),
            usage_description: None,
            geometry: geometry_value,
            area_hectares: area_ha,
            official_area_ha: area_ha,
            source_dataset: Some("INVEKOS".to_string()),
...
#[async_trait]
impl LpisProvider for InvekosProvider {
    ...
    async fn list_parcels(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| format!("INVEKOS request failed: {}", e))?;

        if !response.status().is_success() {
            return Err(format!("INVEKOS API error: {}", response.status()));
        }
    ...
    async fn get_parcel(
        ...
        let query = agrocore_shared::lpis::LpisQuery {
            country: Some(LpisCountry::Other),
    ...
    async fn search_near_point(
        ...
        let url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=INVEKOS&outputFormat=application/json&CQL_FILTER={}&count=100",
            self.base_url,
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| format!("INVEKOS spatial search failed: {}", e))?;
    ...
    async fn validate_geometry(
        ...
        Ok(agrocore_shared::lpis::LpisValidationResult {
            ...
            warnings: if is_empty {
                vec!["No matching INVEKOS parcels found".to_string()]
            } else {
                vec![]
            },
        })
...
impl Default for InvekosProvider {
    fn default() -> Self {
        Self::new()
    }
}
