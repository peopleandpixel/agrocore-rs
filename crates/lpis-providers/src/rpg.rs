...
struct RpgParcel {
    #[serde(rename = "id")]
    id: String,
    #[serde(rename = "code_culture")]
    code_culture: Option<String>,
...
impl RpgProvider {
    ...
    fn build_query_url(&self, query: &agrocore_shared::lpis::LpisQuery) -> String {
        let mut url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=RPG&outputFormat=application/json",
            self.base_url
        );

        let mut filters = Vec::new();

        if let Some(commune) = &query.municipality {
            filters.push(format!("commune='{}'", commune));
        }

        if let Some(dept) = &query.province {
            filters.push(format!("departement='{}'", dept));
        }

        if let Some(culture) = &query.reference {
            filters.push(format!("code_culture='{}'", culture));
        }
    ...
    fn gml_to_polygon(&self, gml: &GmlGeometry) -> Result<Polygon<f64>, RpgError> {
        ...
        if !coords.len().is_multiple_of(2) {
            return Err(RpgError::Geometry("Invalid coordinate count".to_string()));
        }
    ...
    fn parcel_to_lpis(
        ...
        parcel: &RpgParcel,
        tenant_id: Uuid,
    ) -> Result<agrocore_shared::lpis::LpisParcel, RpgError> {
        let polygon = self.gml_to_polygon(&parcel.geometrie)?;
        let geometry = Geometry::Polygon(polygon);

        let geo_json: GeoJson = GeoJsonGeometry::from(&geometry).into();
        let geometry_value =
            serde_json::to_value(geo_json).map_err(|e| RpgError::Geometry(e.to_string()))?;

        let area_ha = parcel.surface.map(|a| a / 10000.0);
        ...
        Ok(agrocore_shared::lpis::LpisParcel {
            ...
            province: parcel.departement.clone(),
            municipality: parcel.commune.clone(),
            aggregate: None,
            zone: None,
            polygon: None,
            parcel: Some(parcel.id.clone()),
            enclosure: None,
            usage_code: parcel.code_culture.clone(),
            usage_description: None,
            geometry: geometry_value,
            area_hectares: area_ha,
            official_area_ha: area_ha,
            source_dataset: Some("RPG".to_string()),
...
#[async_trait]
impl LpisProvider for RpgProvider {
    ...
    async fn search_near_point(
        ...
        let url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=RPG&outputFormat=application/json&CQL_FILTER={}&count=100",
            self.base_url,
    ...
    async fn validate_geometry(
        ...
        Ok(agrocore_shared::lpis::LpisValidationResult {
            ...
            warnings: if is_empty {
                vec!["No matching RPG parcels found".to_string()]
            } else {
                vec![]
            },
        })
