...
struct IlpisParcel {
    ...
    #[serde(rename = "nuts2")]
    nuts2: Option<String>,
    #[serde(rename = "concelho")]
    concelho: Option<String>,
    #[serde(rename = "freguesia")]
    freguesia: Option<String>,
    #[serde(rename = "parcela")]
    parcela: Option<String>,
    #[serde(rename = "cultura")]
    cultura: Option<String>,
    #[serde(rename = "area")]
    area: Option<f64>,
...
impl IlpisProvider {
    ...
    fn build_query_url(&self, query: &agrocore_shared::lpis::LpisQuery) -> String {
        let mut url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=ILPIS&outputFormat=application/json",
            self.base_url
        );
        ...
        if let Some(concelho) = &query.municipality {
            filters.push(format!("concelho='{}'", concelho));
        }

        if let Some(freg) = &query.aggregate {
            filters.push(format!("freguesia='{}'", freg));
        }

        if let Some(cultura) = &query.reference {
            filters.push(format!("cultura='{}'", cultura));
        }
    ...
    fn gml_to_polygon(&self, gml: &GmlGeometry) -> Result<Polygon<f64>, IlpisError> {
        ...
        if !coords.len().is_multiple_of(2) {
            return Err(IlpisError::Geometry("Invalid coordinate count".to_string()));
        }
    ...
    fn parcel_to_lpis(
        ...
        parcel: &IlpisParcel,
        tenant_id: Uuid,
    ) -> Result<agrocore_shared::lpis::LpisParcel, IlpisError> {
        let polygon = self.gml_to_polygon(&parcel.geometria)?;
        let geometry = Geometry::Polygon(polygon);

        let geo_json: GeoJson = GeoJsonGeometry::from(&geometry).into();
        let geometry_value =
            serde_json::to_value(geo_json).map_err(|e| IlpisError::Geometry(e.to_string()))?;

        let area_ha = parcel.area.map(|a| a / 10000.0);
        ...
        Ok(agrocore_shared::lpis::LpisParcel {
            ...
            municipality: parcel.concelho.clone(),
            aggregate: parcel.freguesia.clone(),
            zone: None,
            polygon: parcel.parcela.clone(),
            parcel: parcel.parcela.clone(),
            enclosure: None,
            usage_code: parcel.cultura.clone(),
            usage_description: None,
            geometry: geometry_value,
            area_hectares: area_ha,
            official_area_ha: area_ha,
            source_dataset: Some("iLPIS".to_string()),
...
#[async_trait]
impl LpisProvider for IlpisProvider {
    ...
    async fn list_parcels(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| format!("iLPIS request failed: {}", e))?;

        if !response.status().is_success() {
            return Err(format!("iLPIS API error: {}", response.status()));
        }
    ...
    async fn search_near_point(
        ...
        let url = format!(
            "{}?service=WFS&version=2.0.0&request=GetFeature&typeNames=ILPIS&outputFormat=application/json&CQL_FILTER={}&count=100",
            self.base_url,
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| format!("iLPIS spatial search failed: {}", e))?;
    ...
    async fn validate_geometry(
        ...
        Ok(agrocore_shared::lpis::LpisValidationResult {
            ...
            warnings: if is_empty {
                vec!["No matching iLPIS parcels found".to_string()]
            } else {
                vec![]
            },
        })
