        ...
        .await
    {
        Ok(loc) => {
            ...
            let current_ids: HashSet<Uuid> =
                current_objects.iter().map(|object| object.id).collect();
            let previous_ids: HashSet<Uuid> =
                previous_objects.iter().map(|object| object.id).collect();
                ...
                .await
            {
                Ok(mut orders) => {
                    for mut order in orders.drain(..) {
                            ...
                            if let Err(e) = state
                                ...
                                .update(
                                    ...
                                    agrocore_domain::entities::order::UpdateOrderDto {
                                        ...
                                        assigned_worker_ids: Some(
                                            order.assigned_worker_ids.clone(),
                                        ),
                                        ..Default::default()
                                    },
                                    auth.0.user_id,
                        ...
                        let event =
                            Event::new("api".into(), GlobalEvent::OrderUpdated(order.clone()));
