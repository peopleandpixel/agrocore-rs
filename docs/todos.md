...
## Phase 3: Spezialkulturen & Dauerkulturen — ABGESCHLOSSEN

- [x] Site Entity für mehrjährige Kulturen (DOC Area, Vintage, Grape Variety)
...
- [ ] **Integration-Tests:** Ausweitung der API-Tests auf Fachmodule (Compliance, Olive, Site).
...
- [x] MongoDB Repositories für neue Entities (Site, Olive, Water, Compliance, Workforce) implementiert.
...
### UI Restarbeit
- [x] Sekundäre Buttons in `analytics`, `resources`, `compliance`, `weather`, `livestock`, `finance` und `settings` sind jetzt mit echten API-Aktionen oder echten Navigationszielen hinterlegt.

### Gefundene Bugs / Optimierungen
- [x] **Soft-Delete wird nicht ausgefiltert:** `MongoRepository::find_by_id` und `find_all` filtern jetzt `is_active != false`. Gelöschte Datensätze tauchen nicht mehr in Listen und Detailansichten auf.
- [x] **Reporting-Exporte überspringen die erste Seite:** Die Export-Pagination nutzt jetzt `page: Some(0)` und enthält damit die ersten Datensätze in Excel/GeoJSON-Exporten.
- [x] **Hardcodierte Fallback-Hosts entfernen:** Die Runtime-Fallbacks für MongoDB/NATS zeigen jetzt auf `localhost` statt auf feste LAN-IPs.
- [x] **JWT-Secret nicht pro Request auslesen:** `JWT_SECRET` wird jetzt einmalig gecacht und nicht mehr pro Request neu aus dem Environment gelesen.
- [x] **Pagination-Queries sind teuer:** Relevante Collections haben jetzt `tenant_id + is_active + updated_at` Indizes für die shared Pagination-Queries.
